...
interface EditMetadataModalProps {
  ...
  metadata: {
    ...
    // Record<machine_key, {label, value, url?}>
    extra: Record<string, ExtraEntry>;
  };
  onSave: (updated: EditMetadataModalProps["metadata"]) => Promise<void>;
}

/** Normalize machine keys: lower_snake_case, alnum+underscore only */
...
/** Small helper to deep clone safely */
...
export default function EditMetadataModal({
  ...
  const [form, setForm] = useState<EditMetadataModalProps["metadata"]>(
    deepClone(metadata)
  );

  // Keep local form in sync if metadata prop changes while open
  useEffect(() => {
    if (open) setForm(deepClone(metadata));
  }, [open, metadata]);

  const extraArray = useMemo(() => {
    // Convert Record to editable array of rows with explicit key field
  ...
  const setExtraFromArray = (rows: Array<{ key: string } & ExtraEntry>) => {
    const record: Record<string, ExtraEntry> = {};
    for (const row of rows) {
      const safeKey = normalizeKey(row.key || row.label || "field");
      if (!safeKey) continue;
      record[safeKey] = {
        label: row.label?.trim() || row.key || safeKey,
        value: row.value ?? "",
        url: row.url?.trim() ? row.url.trim() : undefined,
      };
    }
  ...
  // ------- Extra metadata editing (with editable key) -------
  const updateExtraRow = (
    rowIndex: number,
    field: "key" | "label" | "value" | "url",
    value: string
  ) => {
    const rows = deepClone(extraArray);
    rows[rowIndex][field] = field === "key" ? value : value;
    // live-normalize the key for immediate feedback but keep what user typed
    if (field === "key") {
      const norm = normalizeKey(value);
      rows[rowIndex].key = norm;
      if (!rows[rowIndex].label || rows[rowIndex].label === "" || rows[rowIndex].label === rows[rowIndex].key) {
        // If label was empty or just mirroring the key, keep it human-friendly
        rows[rowIndex].label = value.trim() || norm;
      }
    }
  ...
  const addExtra = () => {
    const rows = deepClone(extraArray);
    const base = `field_${rows.length + 1}`;
    rows.push({
      key: base,
      label: "New Field",
      value: "",
  ...
  const handleSubmit = async () => {
    ...
    // Rehydrate extra from array (already normalized via setExtraFromArray on edits)
    const cleanedExtra = deepClone(form.extra || {});
    // Final pass: trim values
    for (const k of Object.keys(cleanedExtra)) {
      ...
      if (!entry.label && k) entry.label = k; // fallback
    }
        ...
        <div className="mb-6">
          ...
          {(form.datasetSources || []).map((src, idx) => (
            <div key={idx} className="flex flex-col md:flex-row gap-2 mb-2">
              <input
                type="text"
                placeholder="Name (e.g., HDX Philippines Portal)"
                value={src.name}
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                  <div className="md:col-span-3">
                    <label className="block text-xs font-medium text-gray-600">
                      Key (machine, snake_case)
                    </label>
                    <input
                      type="text"
                      value={row.key}
                      onChange={(e) =>
                        updateExtraRow(idx, "key", e.target.value)
                      }
                      className="w-full border rounded p-2 text-sm"
                      placeholder="official_languages"
                    />
                  </div>
                  <div className="md:col-span-3">
                    <label className="block text-xs font-medium text-gray-600">
                    ...
                    <input
                      ...
                      value={row.label}
                      onChange={(e) =>
                        updateExtraRow(idx, "label", e.target.value)
                      }
                      className="w-full border rounded p-2 text-sm"
                  ...
                  <div className="md:col-span-4">
                    ...
                    <input
                      ...
                      value={row.value}
                      onChange={(e) =>
                        updateExtraRow(idx, "value", e.target.value)
                      }
                      className="w-full border rounded p-2 text-sm"
                      placeholder="Filipino, English"
                    />
                  </div>
                  <div className="md:col-span-1">
                    ...
                    <input
                      ...
                      value={row.url || ""}
                      onChange={(e) =>
                        updateExtraRow(idx, "url", e.target.value)
                      }
                      className="w-full border rounded p-2 text-sm"
                  ...
                  <div className="md:col-span-1 flex items-end">
                    <button
                      ...
                      className="w-full text-red-500 hover:text-red-700 px-2 py-2 rounded border border-transparent hover:border-red-200"
