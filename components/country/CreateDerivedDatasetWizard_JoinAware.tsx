...
export default function CreateDerivedDatasetWizard_JoinAware({ open, onClose, countryIso, onCreated }: Props) {
  ...
  const [target, setTarget] = useState("ADM3");
  const [warn, setWarn] = useState<string | null>(null);
  const [round, setRound] = useState(0), [unit, setUnit] = useState(""), [title, setTitle] = useState("");
  const [rows, setRows] = useState<any[]>([]), [loading, setLoading] = useState(false);
  ...
  /** Restrict join fields based on dataset type **/
  ...
  /** Auto-detect join level & warn on mismatch **/
  useEffect(() => {
    if (!aMeta && !bMeta) return;
    const aL = aMeta?.admin_level || "", bL = bMeta?.admin_level || "";
    const levels = ["ADM0", "ADM1", "ADM2", "ADM3", "ADM4", "ADM5"];
    const idxA = levels.indexOf(aL.toUpperCase());
    const idxB = levels.indexOf(bL.toUpperCase());
    const deeper = idxA > idxB ? aL : bL;
    if (deeper) setTarget(deeper);
      ...
      <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[["A", aId, setAId, aMeta, joinA, setJoinA, fieldsA],
              ["B", bId, setBId, bMeta, joinB, setJoinB, fieldsB]].map(([l, id, sId, meta, join, sJoin, fields]: any) => (
              <div key={l}>
              ...
              {rows.length === 0
                ...
                : <table className="min-w-full text-sm">
                    <thead className="bg-gray-50 text-gray-600 sticky top-0">
                      <tr><th className="px-3 py-2 text-left">Admin Name</th><th className="px-3 py-2 text-left">PCode</th><th className="px-3 py-2 text-right">A</th><th className="px-3 py-2 text-right">B</th><th className="px-3 py-2 text-right">Derived</th></tr>
                    </thead>
                    <tbody>{rows.map((r, i) =>
                      <tr key={i} className="border-t"><td className="px-3 py-1.5">{r.name}</td><td className="px-3 py-1.5">{r.key}</td>
                        <td className="px-3 py-1.5 text-right">{r.a}</td><td className="px-3 py-1.5 text-right">{r.b}</td>
                        <td className="px-3 py-1.5 text-right">{r.derived}</td></tr>)}
                    </tbody>
                  </table>}
            </div>
          </div>
        ...
        <div className="p-4 border-t flex justify-end gap-2">
          ...
          <button onClick={handleCreate} disabled={loading || !aMeta || !bMeta}
            className="bg-blue-600 text-white px-4 py-2 rounded">{loading ? "Creating…" : "Create"}</button>
        </div>
      </div>
    </div>
  );
