...
export default function CreateDerivedDatasetWizard_JoinAware({ open, onClose, countryIso }: Props) {
  ...
  const datasets = useMemo(() =>
    allDatasets.filter(d =>
      (incCore && d.source === "core") || (incOther && d.source === "other") || (incDerived && d.source === "derived")
  ...
  const [previewRows, setPreviewRows] = useState<any[]>([]), [peekRows, setPeekRows] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const other = (await supabase.from("dataset_metadata")
        .select("id,title,admin_level").eq("country_iso", countryIso)
      ...
      const derived = (await supabase.from("view_derived_dataset_summary")
      ...
      const grouped: TaxonomyMap = tax.reduce((a: TaxonomyMap, t: any) => {
        (a[t.category] ||= []).push(t.name); return a;
      }, {});
      setTaxonomy(grouped);
      const c: Record<string, boolean> = {}, t: Record<string, Record<string, boolean>> = {};
      Object.keys(grouped).forEach(k => { c[k] = false; t[k] = {}; grouped[k].forEach(n => t[k][n] = false); });
      setCatChecked(c); setTermChecked(t);
    })();
  }, [open, countryIso]);
  ...
  async function doPeek(ds: DatasetOption | null, col: string) {
    ...
    if (error || !rows.length) {
      const fb = await supabase.from(ds.table_name).select("pcode,name,population").limit(6);
      rows = fb.data ?? [];
    }
    if (rows.length) { setPeekRows(rows); setPeekTitle(`${ds.title} (${col})`); }
  }
  ...
  async function previewJoin() {
    const { data, error } = await supabase.rpc("simulate_join_preview_autoaggregate", {
      ...
      p_country: countryIso, p_target_level: targetLevel,
      p_method: method, p_col_a: colA,
  ...
  async function save() {
    ...
    const terms = Object.entries(termChecked)
      .flatMap(([c, m]) => Object.entries(m).filter(([, v]) => v).map(([n]) => `${c}:${n}`));
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
      <div className="bg-white rounded w-[980px] max-h-[95vh] overflow-y-auto p-4 text-sm space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="text-xs text-gray-600">Derived = A.{colA} {symbol(method)} {useScalarB?scalarB:`B.${colB}`} → {targetLevel}</div>
        ...
        <div className="max-h-44 overflow-y-auto border rounded">
          <table className="w-full text-xs"><thead className="bg-gray-100 sticky top-0"><tr><th className="text-left p-1">pcode</th><th className="text-left p-1">name</th><th className="text-right p-1">A</th><th className="text-right p-1">B</th><th className="text-right p-1">Derived</th></tr></thead>
          <tbody>{previewRows.map((r:any,i:number)=>(
            <tr key={i} className="odd:bg-gray-50"><td className="p-1">{r.out_pcode}</td><td className="p-1">{r.place_name}</td>
            <td className="p-1 text-right">{r.a}</td><td className="p-1 text-right">{r.b}</td>
            <td className="p-1 text-right">{typeof r.derived==="number"?r.derived.toFixed(decimals):r.derived}</td></tr>
          ))}{!previewRows.length&&<tr><td className="p-2 text-center text-gray-500" colSpan={5}>No preview yet</td></tr>}</tbody></table>
        </div>
        {!!peekRows.length && (
          <div className="max-h-32 overflow-y-auto border rounded">
            <div className="text-xs font-semibold px-2 py-1 bg-gray-50">{peekTitle}</div>
            <table className="w-full text-xs"><thead className="bg-gray-100 sticky top-0"><tr><th className="text-left p-1">pcode</th><th className="text-left p-1">name</th><th className="text-right p-1">value</th></tr></thead>
            <tbody>{peekRows.map((r:any,i:number)=>(
              <tr key={i} className="odd:bg-gray-50"><td className="p-1">{r.pcode}</td><td className="p-1">{r.name}</td><td className="p-1 text-right">{r.population??r.value??""}</td></tr>
            ))}</tbody></table>
          </div>
        )}
        <div><div className="font-semibold mb-1">Taxonomy</div>
          <div className="grid grid-cols-4 gap-3">
            {Object.entries(taxonomy).map(([cat,terms])=>(
              <div key={cat} className="border rounded p-2">
                <label className="font-semibold text-sm flex items-center gap-2">
                  <input type="checkbox" checked={!!catChecked[cat]} onChange={e=>setCatChecked(s=>({...s,[cat]:e.target.checked}))}/>{cat}
                </label>
                {catChecked[cat]&&(
                  <div className="mt-1 pl-3 space-y-1">
                    {terms.map(t=>(
                      <label key={t} className="flex items-center gap-2 text-xs">
                        <input type="checkbox" checked={!!termChecked[cat]?.[t]}
                          onChange={e=>{
                            const v=e.target.checked;
                            setTermChecked(s=>{const n={...s};n[cat]={...(n[cat]||{}),[t]:v};return n;});
                          }}/> {t}
                      </label>
                    ))}
                  </div>
