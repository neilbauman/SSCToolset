...
export default function CreateDerivedDatasetWizard_JoinAware({ open, onClose, countryIso, onCreated }: Props) {
  ...
  const [aScalarValue, setAScalarValue] = useState<number | null>(null), [bScalarValue, setBScalarValue] = useState<number | null>(null);
  ...
  const [targetLevel, setTargetLevel] = useState("ADM3"), [method, setMethod] = useState<Method>("multiply");
  const [roundTo, setRoundTo] = useState(0), [unit, setUnit] = useState(""), [title, setTitle] = useState("");
  const [previewRows, setPreviewRows] = useState<any[]>([]), [loading, setLoading] = useState(false);
  ...
  async function getColumnsForTable(table: string): Promise<string[]> {
  ...
  useEffect(() => { if (aMeta) getColumnsForTable(aMeta.source_table || "dataset_values").then(setFieldsA); }, [aMeta]);
  useEffect(() => { if (bMeta) getColumnsForTable(bMeta.source_table || "dataset_values").then(setFieldsB); }, [bMeta]);

  async function scalar(meta: DatasetInfo) {
    if (meta.dataset_type === "population") {
      const { data: version } = await supabase.from("population_dataset_versions")
        .select("id").eq("country_iso", countryIso).eq("is_active", true).maybeSingle();
      if (!version) return null;
      const { data } = await supabase.from("population_data")
        .select("population").eq("dataset_version_id", version.id).limit(1);
      return data?.[0]?.population ?? null;
    }
    const { data } = await supabase.from("dataset_values")
      .select("value").eq("dataset_id", meta.id).limit(1);
  ...
  useEffect(() => {
    ...
    setAScalar(s); if (s) scalar(aMeta).then(setAScalarValue);
  }, [aMeta?.id]);
  useEffect(() => {
    if (!bMeta) return;
    const s = (bMeta.admin_level || "").toUpperCase() === "ADM0" && (bMeta.record_count ?? 0) <= 1;
    setBScalar(s); if (s) scalar(bMeta).then(setBScalarValue);
  }, [bMeta?.id]);

  function compute(m: Method, a: number | null, b: number | null) {
    switch (m) {
      case "multiply": return a != null && b != null ? a * b : null;
      case "ratio": return a != null && b ? a / b : null;
      case "sum": return (a ?? 0) + (b ?? 0);
      case "difference": return (a ?? 0) - (b ?? 0);
      default: return null;
    }
  }
  ...
  async function rows(meta: DatasetInfo, j: string) {
    ...
    if (meta.dataset_type === "population") {
      ...
      if (!version) return [];
      const { data } = await supabase.from("population_data")
        .select("pcode as key, population as value")
        .eq("dataset_version_id", version.id).limit(500);
      return data || [];
    }
    const { data } = await supabase.from("dataset_values")
      .select(`${j} as key, value`).eq("dataset_id", meta.id).limit(500);
    return data || [];
  }
  ...
  async function previewJoin() {
    ...
    const { data: admins } = await supabase.from("admin_units")
      .select("pcode,name").eq("country_iso", countryIso).eq("level", targetLevel).limit(12);
    const out = (admins || []).map((adm: any) => {
      ...
      return { name: adm.name, key: adm.pcode, a, b, derived: d != null ? Number(d.toFixed(roundTo)) : null };
    });
    setPreviewRows(out);
  }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center">
      <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-5 border-b"><h2 className="text-2xl font-semibold">Create Derived Dataset</h2></div>

        <div className="p-5 space-y-4 overflow-y-auto">
          <div className="grid md:grid-cols-2 gap-4">
            {[["A", aId, setAId, aMeta, joinA, setJoinA, fieldsA, aScalar],
              ["B", bId, setBId, bMeta, joinB, setJoinB, fieldsB, bScalar]].map(([lbl, id, setId, meta, join, setJoin, fields, scalar]: any) =>
              <div key={lbl}>
          ...
          {aMeta && bMeta &&
            <div className="text-xs text-gray-600 border p-2 rounded bg-gray-50">
              Result: <strong>A</strong> ({aMeta.title}) <em>{method}</em> <strong>B</strong> ({bMeta.title})
              → target <strong>{targetLevel}</strong>
            </div>}

          <div className="grid md:grid-cols-3 gap-4">
            <input type="number" value={roundTo} onChange={(e) => setRoundTo(Number(e.target.value))} className="border rounded px-2 py-1.5" placeholder="Round" />
              ...
              {previewRows.length === 0 ?
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50 text-gray-600 sticky top-0">
                    <tr><th className="px-3 py-2 text-left">Admin Name</th>
                      <th className="px-3 py-2 text-left">PCode</th>
                      <th className="px-3 py-2 text-right">A</th>
                      <th className="px-3 py-2 text-right">B</th>
                      <th className="px-3 py-2 text-right">Derived</th></tr>
                  </thead>
