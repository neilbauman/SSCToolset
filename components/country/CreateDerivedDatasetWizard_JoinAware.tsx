...
export default function CreateDerivedDatasetWizard_JoinAware({ countryIso, defaultAdminLevel = 'ADM3', defaultYear = new Date().getFullYear(), onClose }: Props) {
  ...
  const [adminLevel, setAdminLevel] = useState(defaultAdminLevel); const [year, setYear] = useState<number>(defaultYear);
  const [method, setMethod] = useState<Method>('ratio'); const [datasets, setDatasets] = useState<DatasetOption[]>([]); const [taxonomy, setTaxonomy] = useState<TaxonomyTerm[]>([]);
  const [datasetA, setDatasetA] = useState<DatasetOption | null>(null); const [datasetB, setDatasetB] = useState<DatasetOption | null>(null);
  const [colA, setColA] = useState(''); const [colB, setColB] = useState(''); const [useScalarB, setUseScalarB] = useState(false); const [scalarB, setScalarB] = useState<number | null>(null);
  const [decimals, setDecimals] = useState(0); const [preview, setPreview] = useState<PreviewRow[]>([]); const [loadingPreview, setLoadingPreview] = useState(false); const [saving, setSaving] = useState(false);
  const [selectedTaxonomyIds, setSelectedTaxonomyIds] = useState<string[]>([]); const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  ...
  const [colsA,setColsA]=useState<string[]>([]),[colsB,setColsB]=useState<string[]>([]);useEffect(()=>{if(datasetA?.table)fetchCols(datasetA.table).then(setColsA).catch(()=>setColsA([]));},[datasetA]);useEffect(()=>{if(!datasetB?.table||useScalarB)return setColsB([]);fetchCols(datasetB.table).then(setColsB).catch(()=>setColsB([]));},[datasetB,useScalarB]);
...
function Th({children}:{children:any}){return<th className="px-3 py-2 text-xs font-semibold text-gray-600">{children}</th>}
function Td({children}:{children:any}){return<td className="px-3 py-2">{children}</td>}
function MultiSelect({label,options,values,onChange}:{label:string;options:{value:string;label:string}[];values:string[];onChange:(v:string[])=>void;}){
  return(<div><label className="text-sm font-medium">{label}</label><div className="mt-1 max-h-40 overflow-auto border rounded p-2">{options.length===0?<div className="text-sm text-gray-500">No options</div>:options.map(o=>{const c=values.includes(o.value);return(<label key={o.value} className="block text-sm"><input type="checkbox" checked={c} onChange={e=>onChange(e.target.checked?[...values,o.value]:values.filter(v=>v!==o.value))}/> {o.label}</label>)})}</div></div>);
}
function DatasetPicker({title,grouped,dataset,setDataset,cols,col,setCol,useScalar,setUseScalar,scalarValue,setScalarValue}:any){
  return(<div className="border p-3 rounded"><div className="flex justify-between mb-2"><span className="text-sm font-medium">{title}</span>{setUseScalar&&<label className="text-sm"><input type="checkbox" checked={useScalar} onChange={e=>setUseScalar(e.target.checked)}/> Scalar</label>}</div>
  {!useScalar?<><select className="w-full border rounded p-2" value={dataset?.table??''} onChange={e=>{const t=e.target.value;const found=grouped.Core.find((d:DatasetOption)=>d.table===t)||grouped.Other.find((d:DatasetOption)=>d.table===t)||grouped.Derived.find((d:DatasetOption)=>d.table===t)||null;setDataset(found);}}>
    <option value="">Select dataset…</option>{['Core','Other','Derived'].map(g=><optgroup key={g} label={g}>{grouped[g].map((d:DatasetOption)=><option key={d.table} value={d.table}>{d.label}</option>)}</optgroup>)}</select>
    {cols.length>0&&<select className="mt-2 w-full border rounded p-2" value={col} onChange={e=>setCol(e.target.value)}><option value="">Select column…</option>{cols.map((c:string)=><option key={c}>{c}</option>)}</select>}</>
  :<input type="number" className="w-full border rounded p-2" placeholder="Scalar value" value={scalarValue??''} onChange={e=>setScalarValue(e.target.value?parseFloat(e.target.value):null)}/>}</div>);
}
