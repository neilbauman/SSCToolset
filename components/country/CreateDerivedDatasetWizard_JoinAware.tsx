...
export default function CreateDerivedDatasetWizard_JoinAware({
  open, onClose, countryIso, onCreated
}: { open: boolean; onClose: () => void; countryIso: string; onCreated?: (id?: string) => void; }) {
  ...
  const [aJoin, setAJoin] = useState(""), [bJoin, setBJoin] = useState("");
  ...
  // Load datasets (population, admin, optional GIS)
  ...
  async function loadDatasets() {
    ...
    const { data: pop } = await supabase.from("view_population_country_active_summary").select("population_title,population_year").eq("country_iso", countryIso);
    if (pop && pop.length > 0) all.push({ id: "population_data", title: pop[0].population_title || "Population Dataset", dataset_type: "population", admin_level: "ADM3", record_count: null });
  ...
  async function fetchFields(id: string, setter: (v: string[]) => void) {
    if (!id) return setter([]);
    const table = id === "population_data" ? "population_data" : "dataset_values";
    const { data, error } = await supabase.rpc("get_table_columns", { table_name: table });
    if (error || !data) setter(["pcode", "admin_pcode", "id"]);
    else setter(data.map((f: { column_name: string }) => f.column_name));
  }
  useEffect(() => { if (a) fetchFields(a, setAFields); }, [a]);
  useEffect(() => { if (b) fetchFields(b, setBFields); }, [b]);

  useEffect(() => {
    const lv = ["ADM0", "ADM1", "ADM2", "ADM3", "ADM4"];
    const idxA = lv.indexOf(aLevel), idxB = lv.indexOf(bLevel);
    setTarget(idxA >= idxB ? aLevel : bLevel);
    setWarn(aLevel !== bLevel ? `⚠️ Joining ${aLevel} with ${bLevel} may require aggregation.` : null);
  }, [aLevel, bLevel]);
  ...
  async function handleCreate() {
    ...
    try {
      const { data, error } = await supabase.rpc("create_simple_derived_dataset_v2", {
        ...
        p_title: title || `${aMeta.title} × ${bMeta.title}`, p_method: method, p_admin_level: target
      });
        ...
        <div className="p-5 overflow-y-auto space-y-5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[["A", a, setA, aMeta, aJoin, setAJoin, aFields, aLevel, setALevel],
              ["B", b, setB, bMeta, bJoin, setBJoin, bFields, bLevel, setBLevel]].map(([label, id, setId, meta, join, setJoin, fields, lvl, setLvl]: any) => (
              <div key={label}>
                ...
                <select value={id} onChange={e => setId(e.target.value)} className="w-full border rounded px-2 py-1.5">
                  <option value="">Select…</option>{datasets.map((d: Dataset) => <option key={d.id} value={d.id}>{d.title}</option>)}
                </select>
                ...
                {fields.length > 0 && (
                  <div className="mt-1">
                    ...
                    <select value={join} onChange={e => setJoin(e.target.value)} className="w-full border rounded px-2 py-1 text-xs">
                      <option value="">Select field</option>{fields.map((f: string, i: number) => <option key={i}>{f}</option>)}
                    </select>
                  </div>
                )}
                ...
                <select value={lvl} onChange={e => setLvl(e.target.value)} className="w-full border rounded px-2 py-1 text-xs">
                  {["ADM0", "ADM1", "ADM2", "ADM3", "ADM4"].map((l: string) => <option key={l}>{l}</option>)}
                </select>
              </div>
            ))}
          </div>
            ...
            {rows.length === 0
              ...
              : <table className="min-w-full text-xs">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr><th className="px-2 py-1 text-left">Name</th><th>PCode</th><th>A</th><th>B</th><th>Derived</th></tr>
                  </thead>
                  <tbody>{rows.map((r, i) =>
                    <tr key={i} className="border-t"><td className="px-2 py-1">{r.name}</td><td>{r.pcode}</td><td>{r.a}</td><td>{r.b}</td><td>{r.derived}</td></tr>)}</tbody>
                </table>}
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4 items-end">
            <div>
              ...
              <select value={c} onChange={e => setC(e.target.value)} className="w-full border rounded px-2 py-1 text-xs">
                <option value="">None</option>{datasets.map((d: Dataset) => <option key={d.id} value={d.id}>{d.title}</option>)}
              </select>
            </div>
            {c && <>
              <div>
              ...
              <div>
                ...
                <select value={aggMode} onChange={e => setAggMode(e.target.value as AggMode)} className="w-full border rounded px-2 py-1 text-xs">
                  <option value="sum">Sum</option><option value="mean">Mean</option><option value="copy_down">Copy Down</option>
                </select>
              </div>
            </>}
          </div>
          ...
          <div className="flex gap-2 mt-2">
            {(["multiply", "ratio", "sum", "difference"] as Method[]).map((m: Method) =>
