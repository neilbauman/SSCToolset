...
export default function CreateDerivedDatasetWizard_JoinAware({ open, onClose, countryIso, onCreated }: { open: boolean; onClose: () => void; countryIso: string; onCreated?: (id?: string) => void; }) {
  ...
  // Fetch joinable fields per dataset
  async function fetchFields(id:string,setter:(v:string[])=>void){
    ...
    if(error||!data)setter(["pcode","admin_pcode","id"]); else setter(data.map((f:any)=>f.column_name));
  }
  useEffect(()=>{ if(a)fetchFields(a,setAFields); },[a]);
  useEffect(()=>{ if(b)fetchFields(b,setBFields); },[b]);

  // Target level / warnings
  useEffect(() => {
  ...
  <div className="p-5 overflow-y-auto space-y-5">
    ...
    <div className="grid md:grid-cols-2 gap-4">
      {[["A",a,setA,aMeta,aJoin,setAJoin,aFields,aLevel,setALevel],["B",b,setB,bMeta,bJoin,setBJoin,bFields,bLevel,setBLevel]].map(([label,id,setId,meta,join,setJoin,fields,lvl,setLvl]:any)=>(
        <div key={label}>
          ...
          {fields.length>0&&(
            <div className="mt-1">
              ...
              <select value={join} onChange={e=>setJoin(e.target.value)} className="w-full border rounded px-2 py-1 text-xs">
                <option value="">Select field</option>{fields.map((f,i)=><option key={i}>{f}</option>)}
              </select>
            </div>
          )}
