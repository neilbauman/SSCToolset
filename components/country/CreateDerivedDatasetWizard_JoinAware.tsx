...
export default function CreateDerivedDatasetWizard_JoinAware({ open, onClose, countryIso }: WizardProps) {
  ...
  const numKey = (r:any)=>Object.keys(r).find(k=>typeof r[k]==="number")||"value";

  useEffect(()=>{if(!open)return;(async()=>{const all:any[]=[];if(includeCore)all.push({id:"core-admin",title:"Administrative Boundaries [Core]",table:"admin_units",type:"Core"},{id:"core-pop",title:"Population Data [Core]",table:"population_data",type:"Core"});if(includeGIS){const{data}=await sb.from("view_gis_status").select("dataset_id,title,admin_level,year").eq("country_iso",countryIso);data?.forEach((g:any)=>all.push({id:g.dataset_id,title:g.title,table:`gis_dataset_${g.dataset_id}`,type:"GIS",admin_level:g.admin_level,year:g.year}));}if(includeOther){const{data}=await sb.from("dataset_metadata").select("id,title,admin_level,year").eq("country_iso",countryIso);data?.forEach((d:any)=>all.push({id:d.id,title:d.title,table:`dataset_${d.id}`,type:"Other",admin_level:d.admin_level,year:d.year}));}if(includeDerived){const{data}=await sb.from("view_derived_dataset_summary").select("derived_dataset_id,derived_title,admin_level,year").eq("country_iso",countryIso);data?.forEach((d:any)=>all.push({id:d.derived_dataset_id,title:d.derived_title,table:`derived_${d.derived_dataset_id}`,type:"Derived",admin_level:d.admin_level,year:d.year}));}setDatasets(all);})();},[open,includeCore,includeOther,includeDerived,includeGIS,countryIso]);
  ...
  const save=async()=>{if(!title||!datasetA)return alert("Add title and dataset A");setSaving(true);
    const{error}=await sb.rpc("create_derived_dataset",{p_country_iso:countryIso,p_title:title,p_description:desc,p_admin_level:targetLevel,p_table_a:datasetA.table,p_table_b:useScalarB?null:datasetB?.table,p_col_a:colA,p_col_b:useScalarB?null:colB,p_use_scalar_b:useScalarB,p_scalar_b_val:useScalarB?scalarB:null,p_method:method,p_decimals:decimals,p_taxonomy_categories:Object.keys(taxonomy),p_taxonomy_terms:Object.values(taxonomy).flat(),p_formula:`${method} of ${colA} and ${useScalarB?scalarB:colB}`});
    if(error){alert("Save failed: "+error.message);setSaving(false);return;}
    await sb.rpc("create_derived_dataset_record",{p_country:countryIso,p_title:title,p_inputs:JSON.stringify({table_a:datasetA.table,table_b:datasetB?.table}),p_method:method,p_expression:`${method} of ${colA} and ${useScalarB?scalarB:colB}`,p_target_level:targetLevel,p_year:new Date().getFullYear()});
