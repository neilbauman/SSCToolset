...
export default function CreateDerivedDatasetWizard_JoinAware({ open, onClose, countryIso, onCreated }: { open: boolean; onClose: () => void; countryIso: string; onCreated?: (id?: string) => void; }) {
  ...
  useEffect(() => {
    if (!open) return;
    (async () => {
      const { data } = await supabase.from("dataset_metadata").select("id,title,dataset_type,admin_level,record_count").eq("country_iso", countryIso);
      setDatasets(data || []);
    })();
  }, [open]);
  ...
  useEffect(() => {
    ...
    setTarget(deeper); if (aLevel !== bLevel) setWarn(`⚠️ Joining ${aLevel} with ${bLevel} may require aggregation.`); else setWarn(null);
  }, [aLevel, bLevel]);
  ...
  function simulateAggregation(base: string, higher: string, mode: AggMode) {
    return mode === "sum" ? `Σ(${base}->${higher})` : mode === "mean" ? `μ(${base}->${higher})` : `↓(${higher}->${base})`;
  }
  ...
  async function handleCreate() {
    ...
    try {
      const { data, error } = await supabase.rpc("create_simple_derived_dataset_v2", {
        p_country_iso: countryIso, p_dataset_a: aMeta.id, p_dataset_b: bMeta.id, p_title: title || `${aMeta.title} × ${bMeta.title}`, p_method: method, p_admin_level: target,
      });
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="w-full max-w-5xl bg-white rounded-lg shadow-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-5 overflow-y-auto space-y-5">
          ...
          <div className="border rounded max-h-48 overflow-y-auto">{rows.length===0?<div className="p-2 text-sm text-gray-500">No preview</div>:
            <table className="min-w-full text-xs">
              <thead className="bg-gray-50 sticky top-0"><tr><th className="px-2 py-1 text-left">Name</th><th>PCode</th><th>A</th><th>B</th><th>Derived</th></tr></thead>
              <tbody>{rows.map((r,i)=><tr key={i} className="border-t"><td className="px-2 py-1">{r.name}</td><td>{r.pcode}</td><td>{r.a}</td><td>{r.b}</td><td>{r.derived}</td></tr>)}</tbody>
            </table>}</div>
          ...
          <div className="text-xs text-gray-600 border rounded p-2 bg-gray-50">
            Formula: <strong>{aMeta?.title||"A"}</strong> {method} <strong>{bMeta?.title||"B"}</strong>{c&&<> with <strong>{cMeta?.title}</strong> ({aggMode})</>}
