...
export default function AddDatasetModal({open,onClose,countryIso,onCreated}:Props){
 ...
 async function handleCreate(){if(!title.trim())return setMsg({type:"err",text:"Title required"});if(!file)return setMsg({type:"err",text:"No CSV selected"});setBusy(true);let metaId:string|null=null;
 try{const src=sourceName||sourceUrl?{name:sourceName||null,url:sourceUrl||null}:null;const ins={title,description:description||null,year:year===""?null:Number(year),unit:unit||null,admin_level:adminLevel,country_iso:countryIso,indicator_id:indicatorId,upload_type:uploadType,source:src?JSON.stringify(src):null};const{data,err}=await supabase.from("dataset_metadata").insert(ins).select("id").single();if(err)throw err;metaId=data?.id;}catch(e:any){setBusy(false);return setMsg({type:"err",text:`Meta save fail: ${e.message}`});}
 try{const parsed=Papa.parse(await file.text(),{header:true,skipEmptyLines:true});if(parsed.errors?.length)throw new Error(parsed.errors[0].message);const rows=(parsed.data as any[]).map(r=>({p:(r.pcode||r.PCode||r.admin_pcode||"").trim(),v:r.value??r.Value??"",u:r.unit??"",n:r.notes??""}));
 let ok=0,skip=0,payload:any[]=[];for(const r of rows){if(!r.p){skip++;continue;}let val:number|null=null;if(r.v===""||r.v==null){skip++;continue;}const n=Number(String(r.v).replace("%",""));if(Number.isFinite(n))val=n;else{skip++;continue;}ok++;payload.push({dataset_id:metaId!,admin_pcode:r.p,value:val,unit:r.u||null,notes:r.n||null});}
 ...
 setBusy(false);setMsg({type:"ok",text:`Upload complete: ${ok} rows${skip?`; ${skip} skipped`:``}`});onCreated&&onCreated();}catch(e:any){setBusy(false);setMsg({type:"err",text:`Upload fail: ${e.message}`});}}
 return(<div className={`fixed inset-0 z-50 ${open?"":"pointer-events-none"}`}><div className={`absolute inset-0 bg-black/40 ${open?"opacity-100":"opacity-0"}`}onClick={handleClose}/><div className={`absolute left-1/2 top-1/2 w-full max-w-3xl -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white shadow-lg ${open?"opacity-100":"opacity-0"}`}><div className="flex justify-between border-b px-5 py-3"><h3 className="text-lg font-semibold text-[color:var(--gsc-gray)]">Add New Dataset</h3><button onClick={handleClose}><X className="w-5 h-5"/></button></div>
 <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
  <div className="border rounded-lg p-3"><label className={LABEL}>Link to Indicator</label>
   <div className="flex gap-2 mb-2"><div className="relative flex-1"><Search className="w-4 h-4 text-gray-400 absolute left-2 top-2.5"/><input className={`${FIELD} pl-8`}placeholder="Search..."value={search}onChange={e=>setSearch(e.target.value)}/></div>
   <select className={FIELD}value={themeFilter}onChange={e=>setThemeFilter(e.target.value)}><option>All</option>{themes.map(t=><option key={t}>{t}</option>)}</select></div>
   <div className="border rounded max-h-56 overflow-auto">{filtered.map(i=><button key={i.id}onClick={()=>setIndicatorId(indicatorId===i.id?null:i.id)}className={`w-full text-left px-3 py-2 border-b hover:bg-gray-50 ${indicatorId===i.id?"bg-green-50":""}`}><div className="font-medium">{i.name}</div><div className="text-xs text-gray-500">{i.theme||"—"} {i.data_type?`• ${i.data_type}`:""}</div></button>)}{!filtered.length&&<div className="p-3 text-sm text-gray-500">No results</div>}</div>
   ...
   <div className="mt-4"><label className={LABEL}>CSV File</label><input ref={fileRef}type="file"accept=".csv"text/csv"onChange={e=>setFile(e.target.files?.[0]||null)}className={FIELD}/><p className="mt-1 text-xs text-gray-500">Columns: pcode,value,(unit,notes)</p></div>
  </div>
 ...
 <div className="flex items-center justify-between border-t px-5 py-3"><div className="text-sm text-gray-600 flex items-center gap-2"><Info className="w-4 h-4 text-gray-400"/>Large CSVs insert in chunks</div>
  <div className="flex gap-2"><button onClick={handleClose}className="px-3 py-2 border rounded-md">Cancel</button><button onClick={handleCreate}disabled={busy}className={GSC_BTN}><Upload className="w-4 h-4"/>{busy?"Uploading...":"Upload Dataset"}</button></div></div>
 {msg&&<div className={`mx-5 mb-4 mt-2 rounded-md border px-3 py-2 text-sm flex items-center gap-2 ${msg.type==="ok"?"border-green-300 bg-green-50 text-green-800":"border-red-300 bg-red-50 text-red-800"}`}>{msg.type==="ok"?<CheckCircle2 className="w-4 h-4"/>:<AlertCircle className="w-4 h-4"/>}<span>{msg.text}</span></div>}
 </div></div>);
}
