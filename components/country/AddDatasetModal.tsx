...
export default function AddDatasetModal({open,onClose,countryIso,onCreated}:Props){
 ...
 const[valueAdm0,setValueAdm0]=useState<string>(""); // New field for ADM0 direct value
 ...
 const reset=()=>{setTitle("");setDescription("");setYear("");setUnit("");setAdminLevel("ADM0");setDataType("numeric");setUploadType("gradient");setSourceName("");setSourceUrl("");setIndicatorId(null);setFile(null);setValueAdm0("");if(fileRef.current)fileRef.current.value="";setMsg(null);};
 const handleClose=()=>{if(busy)return;reset();onClose();};

 async function handleCreate(){
  if(!title.trim())return setMsg({type:"err",text:"Title required"});
  if(adminLevel!=="ADM0"&&!file)return setMsg({type:"err",text:"Please select a CSV"});
  if(adminLevel==="ADM0"&&!valueAdm0.trim())return setMsg({type:"err",text:"Please enter a national value"});
  setBusy(true);
  ...
  }catch(e:any){setBusy(false);return setMsg({type:"err",text:`Metadata failed: ${e.message}`});}

  try{
   if(adminLevel==="ADM0"){
    const val=Number(valueAdm0);
    if(!Number.isFinite(val))throw new Error("Invalid numeric value");
    const {error}=await supabase.from("dataset_values").insert({dataset_id:metaId!,admin_pcode:"ADM0",value:val,unit:unit||null});
    if(error)throw error;
    setBusy(false);setMsg({type:"ok",text:"National-level value saved."});onCreated&&onCreated();return;
   }
   ...
   <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-4">
    ...
    <div className="border rounded-lg p-3">
     <div className="flex items-center justify-between mb-2">
      ...
      <select className="text-sm border rounded px-2 py-1" value={themeFilter} onChange={e=>setThemeFilter(e.target.value)}>
       <option>All</option>{themes.map(t=><option key={t}>{t}</option>)}
      </select>
     </div>
     <div className="relative mb-2">
      <Search className="w-4 h-4 text-gray-400 absolute left-2 top-2.5"/>
      <input className={`${FIELD} pl-8`} placeholder="Search indicator..." value={search} onChange={e=>setSearch(e.target.value)}/>
     </div>
     <div className="border rounded max-h-56 overflow-auto">
      {filtered.map(i=>(
       <button key={i.id} onClick={()=>setIndicatorId(indicatorId===i.id?null:i.id)} className={`w-full text-left px-3 py-2 border-b hover:bg-gray-50 ${indicatorId===i.id?"bg-green-50":""}`}>
        <div className="font-medium flex justify-between"><span>{i.name}</span><span className="text-xs bg-[color:var(--gsc-beige)] px-2 rounded">{i.theme||"—"}</span></div>
        <div className="text-xs text-gray-500">{i.code} {i.data_type?`• ${i.data_type}`:""}</div>
       </button>
      ))}
      {!filtered.length&&<div className="p-3 text-sm text-gray-500">No indicators</div>}
     </div>
    ...
    <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
     ...
     {adminLevel==="ADM0"&&(
      <div><label className={LABEL}>National Value</label><input className={FIELD} value={valueAdm0} onChange={e=>setValueAdm0(e.target.value)} placeholder="Enter numeric value"/></div>
     )}
