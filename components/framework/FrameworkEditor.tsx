...
export default function FrameworkEditor({ versionId }: Props) {
    ...
    return (
      <div className="border rounded-lg mb-3 bg-white shadow-sm">
        <div className="flex items-center gap-2 px-4 py-3 border-b">
          ...
          {pillar.description && <div className="text-sm text-gray-600 ml-2">{pillar.description}</div>}
        </div>
        {open && (
          <div className="p-4">
                ...
                <ul className="mt-2 list-disc pl-5">
                  {theme.subthemes.map(s => (
                    <li key={s.id}>
                      ...
                      {s.description && <span className="text-gray-600 ml-2 text-sm">{s.description}</span>}
                    </li>
                  ))}
                </ul>
              </div>
  ...
  return (
    <div>
      {data.map(p => <PillarRow key={p.id} pillar={p} />)}
      {data.length === 0 && (
