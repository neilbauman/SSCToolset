...
export default function AddLayerModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select(
          "dataset_id, title, country_iso, admin_level, dataset_type, origin"
        )
  ...
  const save = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("Failed to add dataset: " + (e as Error).message);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/40 flex items-center justify-center">
      <div className="w-full max-w-lg bg-white rounded-lg p-4 shadow-lg text-sm">
