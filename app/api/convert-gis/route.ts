...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3️⃣ upload to public gis bucket
    const geoPath = `${country_iso}/${version_id}/layer.geojson`;
    const { error: upErr } = await supabaseServer.storage
      .from("gis")
      .upload(geoPath, geojson, { upsert: true, contentType: "application/geo+json" });
    ...
    // 5️⃣ activate dataset version
    await supabaseServer
      .from("gis_dataset_versions")
      .update({ is_active: true, source: "Server conversion" })
      .eq("id", version_id);

    return NextResponse.json({ ok: true, message: "Conversion complete" });
  } catch (err: any) {
    console.error(err);
