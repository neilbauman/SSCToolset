...
export default function SSCDashboard({
  ...
  const applyToInstance = async (ds: DatasetRow) => {
    // Persist catalog first (in case it was edited elsewhere)
    // Then call correct RPC based on method
    const method = (ds.norm_method || "").toLowerCase();
    ...
    if (method.includes("threshold_bands")) {
      ...
      // Fallback classification RPC if explicitly chosen
      const { error } = await supabase.rpc(
    ...
    // Reload not strictly necessary for preview table, but keeps list fresh
    await loadDatasets();
    alert("Applied to instance.");
  };

  const removeDataset = async (ds: DatasetRow) => {
    if (!confirm(`Remove "${ds.metric}" / "${ds.source_note}" from catalog?`)) return;
    const { error } = await supabase
      .from("ssc_dataset_catalog")
      .delete()
      .eq("metric", ds.metric)
      .eq("source_note", ds.source_note);
    if (error) {
      alert(error.message);
      return;
    }
...
function DatasetTable({
  ...
  loading: boolean;
  datasets: DatasetRow[];
  onInterpret: (ds: DatasetRow) => void;
  onView: (ds: DatasetRow) => void;
  onApply: (ds: DatasetRow) => void;
  onRemove: (ds: DatasetRow) => void;
}) {
  return (
