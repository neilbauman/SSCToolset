...
export default function SSCDashboard({
  ...
  // Apply normalization based on dataset method
  const handleApply = async (
    ...
    try {
      if (norm_method.includes("threshold") || norm_method.includes("band")) {
        console.log("🟩 Applying threshold-based scoring for:", metric);
        ...
        console.log("🟦 Applying gradient-based normalization for:", metric);
        const { error } = await supabase.rpc(
      ...
      console.error("Failed to apply dataset normalization:", err);
    }
  };
          ...
          <header className="px-4 py-2 bg-[color:var(--gsc-green)] text-white flex justify-between items-center rounded-t-lg">
            ...
            <RefreshCw
              ...
              className={`h-4 w-4 cursor-pointer ${
                loading ? "animate-spin text-gray-300" : "hover:rotate-90"
...
function DatasetTable({
  ...
  onView: (ds: any) => void;
  onApply: (metric: string, source: string, norm_method: string) => void;
}) {
  return (
          ...
          {datasets.length ? (
            datasets.map((d) => (
              ...
              >
                ...
                <td className="p-2 text-gray-500">{d.admin_level || "ADM3/ADM4"}</td>
                ...
                <td className="p-2 text-right">
                  ...
                  <button
                    onClick={() =>
                      onApply(d.metric, d.source_note, d.norm_method)
                    }
