...
export default function PopulationPage({ params }: any) {
  ...
  // Fetch population dataset source
  useEffect(() => {
  ...
  // Health checks (placeholder logic for now)
  const hasPopulation = false;
  const hasHouseholds = false;
  const datasetDate = null;
    ...
    <SidebarLayout headerProps={headerProps}>
        ...
        <div className="border rounded-lg p-4 shadow-sm relative">
          ...
          <ul className="text-sm list-disc pl-6">
            <li className={hasPopulation ? "text-green-700" : "text-red-700"}>
              {hasPopulation ? "Population data uploaded" : "No population data"}
            </li>
            <li className={hasHouseholds ? "text-green-700" : "text-yellow-700"}>
              {hasHouseholds ? "Household counts included" : "Household counts missing"}
            </li>
            <li className="text-gray-700">
              {datasetDate
                ? `Dataset year: ${datasetDate}`
                : "Dataset year not defined"}
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="border rounded-lg p-4 shadow-sm">
        <p className="text-sm text-gray-600 italic">
          🚧 Population dataset view coming soon.
        </p>
      </div>
