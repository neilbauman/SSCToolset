...
export default function PopulationPage({ params }: { params: CountryParams }) {
  ...
  const headerProps={title:`${country?.name||countryIso} – Population Data`,group:"country-config",description:"Manage versioned population datasets.",breadcrumbs:(<Breadcrumbs items={[{label:"Dashboard",href:"/dashboard"},{label:"Country Configuration",href:"/country"},{label:country?.name||countryIso,href:`/country/${countryIso}`},{label:"Population"}]}/>) };
