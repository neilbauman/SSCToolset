...
export default function AdminsPage({ params }: { params: CountryParams }) {
  ...
  const renderTree = (n: any, d = 0) => (
    <div key={n.pcode} style={{ marginLeft: d * 16 }}>
      ...
      {expanded.has(n.pcode) &&
        n.children.map((c: any) => renderTree(c, d + 1))}
    </div>
  );
  ...
  return (
    <SidebarLayout headerProps={headerProps}>
                ...
                >
                  ...
                  <td className="text-right flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className="text-[color:var(--gsc-red)] text-xs hover:underline"
                    >
      ...
      <div className="flex justify-between mb-2">
        ...
        <div className="flex gap-2">
          {["table", "tree"].map((m) => (
            <button
              key={m}
              onClick={() => setViewMode(m as "table" | "tree")}
              className={`px-3 py-1 text-sm border rounded ${
                viewMode === m ? "bg-blue-50 border-blue-400" : ""
              }`}
            >
      ...
      {openDelete && (
        <ConfirmDeleteModal
          open
          message={`Remove "${openDelete.title}" and its records?`}
          onClose={() => setOpenDelete(null)}
          onConfirm={() => delVersion(openDelete.id)}
        />
      )}
      {editingVersion && (
