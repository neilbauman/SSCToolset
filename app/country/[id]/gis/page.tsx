...
import { useMap } from "react-leaflet"; // ✅ keep this only — remove any other useMap line
...
export default function GISPage({ params }: { params: CountryParams }) {
  ...
  // ─── Visibility and GeoJSON Loading ──────────
  const toggleLayer = async (layer: GISLayer) => {
    ...
    if (next && !geojsonById[id]) {
      try {
        ...
        console.error("⚠️ Failed to load GeoJSON:", e);
      }
    }
  };
  ...
  const onDelete = async (layer: GISLayer) => {
    ...
    try {
      ...
      setVisible(v => {
        const n = { ...v }; delete n[layer.id]; return n;
      });
      setGeojsonById(g => {
        const n = { ...g }; delete n[layer.id]; return n;
      });
    } catch (e: any) {
