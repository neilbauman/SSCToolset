...
const Step=({title,children,back,next,disable}:any)=>(<div className="space-y-4"><h2 className="text-lg font-semibold">{title}</h2><div className="rounded-xl border p-4">{children}</div><div className="flex justify-between">{back?<button onClick={back} className="border px-3 py-1 rounded-xl flex items-center gap-1"><ArrowLeft className="h-4 w-4"/>Back</button>:<span/>}{next&&<button onClick={next} disabled={disable} className="bg-black text-white px-3 py-1 rounded-xl flex items-center gap-1">Next<ArrowRight className="h-4 w-4"/></button>}</div></div>);

export default function DatasetWizard(){
 const raw=(useParams()?.id??"") as string|string[]; const iso=(Array.isArray(raw)?raw[0]:raw)?.toUpperCase()||"";
 ...
 async function saveMeta(){try{setLoading(true);setErr("");if(!meta.title||!meta.dataset_type||!meta.admin_level||!meta.join_field)throw new Error("Missing required fields");
 ...
 window.location.href=`/country/${iso}/datasets`;return;}setStep(s=>s+1);}catch(e:any){setErr(e.message)}finally{setLoading(false)}};
  ...
  <div className="space-y-4">{err&&<div className="bg-red-50 text-red-700 border p-2 rounded">{err}</div>}
   ...
   {steps[step]==="Meta"&&(<Step title="Dataset details" next={saveMeta}>
    <div className="grid md:grid-cols-2 gap-3">
     <input className="border p-2 rounded" placeholder="Title" value={meta.title} onChange={e=>setMeta({...meta,title:e.target.value})}/>
     <select className="border p-2 rounded" value={meta.dataset_type} onChange={e=>setMeta({...meta,dataset_type:e.target.value})}>
      {["adm0","gradient","categorical"].map(x=><option key={x}>{x}</option>)}</select>
     <select className="border p-2 rounded" value={meta.data_format} onChange={e=>setMeta({...meta,data_format:e.target.value})}>
      {["numeric","percentage","text"].map(x=><option key={x}>{x}</option>)}</select>
     <select className="border p-2 rounded" value={meta.admin_level} onChange={e=>setMeta({...meta,admin_level:e.target.value})}>
      {["ADM0","ADM1","ADM2","ADM3","ADM4","ADM5"].map(x=><option key={x}>{x}</option>)}</select>
     {["join_field","year","unit","source_name","source_url"].map(k=>
      <input key={k} className="border p-2 rounded" placeholder={k.replace("_"," ")} value={(meta as any)[k]}
    ...
    <div className="border rounded p-3 space-y-2 mt-3" style={{borderColor:"var(--gsc-light-gray)"}}>
     ...
     <div className="flex gap-2 items-center">
      <div className="relative grow"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search indicators…" className="w-full rounded border px-8 py-2 text-sm" style={{borderColor:"var(--gsc-light-gray)"}}/><Search className="absolute left-2 top-2.5 h-4 w-4 text-gray-400"/></div>
      <select className="border rounded p-2 text-sm" style={{borderColor:"var(--gsc-light-gray)"}} value={meta.indicator_id} onChange={e=>setMeta({...meta,indicator_id:e.target.value})}>
       <option value="">(none)</option>{indicators.filter(i=>i.name.toLowerCase().includes(q.toLowerCase())).slice(0,300).map(i=><option key={i.id} value={i.id}>{i.name}</option>)}
      </select>
     </div>
     <div className="text-xs text-gray-600">Taxonomy:&nbsp;<span className="font-medium">{taxonomy.category??"—"}</span>{taxonomy.term?<> • <span>{taxonomy.term}</span></>:null}</div>
    </div>
    <div className="flex justify-end mt-3"><button disabled={loading} onClick={saveMeta} className="bg-[var(--gsc-blue)] text-white px-4 py-2 rounded-xl">{loading?<><Loader2 className="h-4 w-4 animate-spin inline mr-2"/>Saving…</>:"Save & Continue"}</button></div>
   </Step>)}

   {steps[step]==="ADM0"&&(<Step title="ADM0 value" back={()=>setStep(s=>s-1)} next={next}><input className="border p-2 rounded w-full" placeholder="Numeric value" value={adm0} onChange={e=>setAdm0(e.target.value)}/></Step>)}
   ...
   {steps[step]==="Upload"&&(<Step title="Upload file & map" back={()=>setStep(s=>s-1)} next={next}>
    ...
    {file&&<button className="mt-2 border px-3 py-1 rounded" onClick={()=>file&&onFile(file)}><Upload className="h-4 w-4 inline mr-1"/>Parse file</button>}
