...
[file,setFile]=useState<File|null>(null),[headers,setHeaders]=useState<string[]>([]),[rows,setRows]=useState<any[]>([]),
[joinColumn,setJoinColumn]=useState(""),[nameColumn,setNameColumn]=useState(""),[categoryCols,setCategoryCols]=useState<string[]>([]),
[categoryMap,setCategoryMap]=useState<{code:string;label:string}[]>([]),[matchInfo,setMatchInfo]=useState<{rate:number;missing:string[]}|null>(null),
[taxonomyIds,setTaxonomyIds]=useState<string[]>([]),[indicatorQuery,setIndicatorQuery]=useState(""),[indicatorList,setIndicatorList]=useState<any[]>([]),
...
async function searchIndicators(){let q=supabase.from("indicator_catalogue").select("id,name,data_type,description,status").order("name");
 if(selectedTerm){const link=await supabase.from("indicator_taxonomy_links").select("indicator_id").eq("taxonomy_id",selectedTerm);
 const ids=(link.data??[]).map(i=>i.indicator_id);if(ids.length)q=q.in("id",ids);}
...
<CreateIndicatorInlineModal open={createIndicatorOpen} onClose={()=>setCreateIndicatorOpen(false)} taxonomyDefault={taxonomyIds} onCreated={async id=>{await supabase.from("indicator_catalogue").update({status:"proposed"}).eq("id",id);await Promise.all(taxonomyIds.map(t=>supabase.from("indicator_taxonomy_links").insert({indicator_id:id,taxonomy_id:t})));setIndicatorId(id);setCreateIndicatorOpen(false);}}/></section>)}
