...
export default function DatasetWizard({countryIso,onClose,onSaved}:Props){
  ...
  const[title,setTitle]=useState(""),[desc,setDesc]=useState(""),[source,setSource]=useState(""),[sourceUrl,setSourceUrl]=useState(""),[year,setYear]=useState<number|''>(""),
        [datasetType,setDatasetType]=useState<"gradient"|"categorical">("gradient"),
        [dataFormat,setDataFormat]=useState<"numeric"|"percentage"|"text">("numeric"),
  ...
  const[file,setFile]=useState<File|null>(null),[headers,setHeaders]=useState<string[]>([]),[rows,setRows]=useState<any[]>([]),
        [joinColumn,setJoinColumn]=useState(""),[nameColumn,setNameColumn]=useState(""),
        [categoryCols,setCategoryCols]=useState<string[]>([]),[categoryMap,setCategoryMap]=useState<{code:string;label:string}[]>([]);
  const[taxonomyIds,setTaxonomyIds]=useState<string[]>([]),[indicatorQuery,setIndicatorQuery]=useState(""),
        [indicatorList,setIndicatorList]=useState<any[]>([]),[indicatorId,setIndicatorId]=useState<string|null>(null),
        [createIndicatorOpen,setCreateIndicatorOpen]=useState(false),
        [groups,setGroups]=useState<string[]>([]),[terms,setTerms]=useState<any[]>([]),
  ...
  async function searchIndicators(){
    let q=supabase.from("indicator_catalogue").select("id,name,data_type,description,status").order("name");
  ...
  async function saveAll(){
    ...
    try{
      ...
      if(datasetType==="categorical"){
        detectCategories();const maps=categoryMap.map(m=>({dataset_id:id,code:m.code,label:m.label,score:null}));
  ...
  const canNext=!!title&&!!datasetType&&!!dataFormat&&!!adminLevel,
        canSave=rows.length>0||(adminLevel==="ADM0"&&!!nationalValue.trim());
      ...
      <div className="p-5 space-y-4 overflow-y-auto">
        ...
        {step===1&&(<section className="space-y-3">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <div><label className={L}>Type</label><select className={F} value={datasetType} onChange={e=>setDatasetType(e.target.value as any)}><option value="gradient">Gradient</option><option value="categorical">Categorical</option></select></div>
        ...
        {step===3&&(<section className="space-y-3">
          ...
          <select multiple className={`${F} h-40`} value={categoryCols} onChange={e=>setCategoryCols([...e.target.selectedOptions].map(o=>o.value))}>
            {headers.filter(h=>h!==joinColumn&&h!==nameColumn).map(h=><option key={h}>{h}</option>)}
          </select>
        ...
        {step===4&&(<section className="space-y-4">
          <div className="flex flex-wrap gap-3">
            ...
            <div><label className={L}>Filter by Term</label>
              <select className={F} value={selectedTerm} onChange={e=>setSelectedTerm(e.target.value)}>
                <option value="">All</option>{terms.filter(t=>!selectedGroup||t.category===selectedGroup).map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-2">
                <input className={F} placeholder="Search..." value={indicatorQuery} onChange={e=>setIndicatorQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchIndicators()}/>
            ...
            <div className="flex items-end">
              <button className={S} onClick={()=>setCreateIndicatorOpen(true)}><Plus className="w-4 h-4"/>New</button>
            </div>
          </div>
          ...
          <CreateIndicatorInlineModal
            ...
            onCreated={async id=>{
              ...
              setIndicatorId(id);setCreateIndicatorOpen(false);
            }}
          />
        </section>)}
