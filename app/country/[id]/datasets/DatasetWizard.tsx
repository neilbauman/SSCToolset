...
const F="w-full border rounded-md px-3 py-2 text-sm outline-none focus:ring-2 ring-[color:var(--gsc-red)]",
      L="block text-xs font-medium text-[color:var(--gsc-gray)] mb-1",
      B="inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm",
      P=`${B} bg-[color:var(--gsc-red)] text-white hover:opacity-90 disabled:opacity-50`,
      S=`${B} border hover:bg-gray-50`;
...
const[matchInfo,setMatchInfo]=useState<{rate:number;missing:string[]}|null>(null);
...
[groups,setGroups]=useState<string[]>([]),[terms,setTerms]=useState<any[]>([]),[selectedGroup,setSelectedGroup]=useState(""),[selectedTerm,setSelectedTerm]=useState("");

const next=()=>setStep(s=>Math.min(s+1,5)),prev=()=>setStep(s=>Math.max(s-1,1));

async function parseCSV(f:File){return new Promise<{headers:string[];rows:any[]}>((res,rej)=>Papa.parse(f,{header:true,dynamicTyping:true,skipEmptyLines:true,complete:r=>res({headers:r.meta.fields??Object.keys(r.data[0]??{}),rows:r.data.slice(0,300)}),error:rej}));}
async function handleFile(e:any){const f=e.target.files?.[0];if(!f)return;const{headers,rows}=await parseCSV(f);setFile(f);setHeaders(headers);setRows(rows);
...
async function checkPcodes(){
 ...
 try{
   const {data:known}=await supabase.from("admin_units").select("pcode").eq("level",adminLevel);
   const knownSet=new Set((known??[]).map((u:any)=>u.pcode?.trim()));
...
async function searchIndicators(){let q=supabase.from("indicator_catalogue").select("id,name,data_type,description,status").order("name");
if(selectedTerm){const link=await supabase.from("indicator_taxonomy_links").select("indicator_id").eq("taxonomy_id",selectedTerm);const ids=(link.data??[]).map(i=>i.indicator_id);if(ids.length)q=q.in("id",ids);}
...
async function saveAll(){setBusy(true);setError(null);
 try{
  ...
  if(datasetType==="gradient"||datasetType==="categorical"){
   const d:any[]=[];
   rows.forEach(r=>{
     if(categoryCols.length){
       categoryCols.forEach(c=>d.push({dataset_id:id,admin_pcode:String(r[joinColumn]??"").trim(),admin_level:adminLevel,category_label:c,value:Number(r[c]??0)}));
     }else{
       const fallback=headers.find(h=>h!==joinColumn&&h!==nameColumn);
       if(fallback)d.push({dataset_id:id,admin_pcode:String(r[joinColumn]??"").trim(),admin_level:adminLevel,value:Number(r[fallback]??0)});
     }
   });
...
{step===1&&(<section className="space-y-3">
 ...
 <div className="grid md:grid-cols-3 gap-3"><div><label className={L}>Type</label><select className={F} value={datasetType} onChange={e=>setDatasetType(e.target.value as any)}><option value="gradient">Gradient</option><option value="categorical">Categorical</option></select></div>
 <div><label className={L}>Format</label><select className={F} value={dataFormat} onChange={e=>setDataFormat(e.target.value as any)}><option value="numeric">Numeric</option><option value="percentage">Percentage</option><option value="text">Text</option></select></div>
...
{step===2&&adminLevel!=="ADM0"&&(<section className="space-y-3">
 ...
 {headers.length>0&&(<div className="grid md:grid-cols-2 gap-3">
  <div><label className={L}>Admin PCode Column</label><select className={F} value={joinColumn} onChange={e=>{setJoinColumn(e.target.value);setMatchInfo(null);}}>{headers.map(h=><option key={h}>{h}</option>)}</select></div>
 ...
 {matchInfo&&(<div className={`p-3 rounded text-sm border ${matchInfo.rate>90?"border-green-200 bg-green-50 text-green-700":matchInfo.rate>40?"border-yellow-200 bg-yellow-50 text-yellow-700":"border-red-200 bg-red-50 text-red-700"}`}>
  {matchInfo.rate>90?"✅":matchInfo.rate>40?"⚠️":"❌"} {matchInfo.rate}% of admin codes matched existing {adminLevel} units.
  {matchInfo.missing.length>0&&<div className="text-xs mt-1">Missing examples: {matchInfo.missing.join(", ")}</div>}
 </div>)}
 {rows.length>0&&(<div className="border rounded p-2 max-h-60 overflow-auto text-xs"><table className="min-w-full"><thead><tr>{headers.map(h=><th key={h} className="text-left px-2 py-1 border-b">{h}</th>)}</tr></thead><tbody>{rows.slice(0,10).map((r,i)=><tr key={i} className="odd:bg-gray-50">{headers.map(h=><td key={h} className="px-2 py-1 border-b">{String(r[h]??"")}</td>)}</tr>)}</tbody></table></div>)}
</section>)}
...
{step===4&&(<section className="space-y-4">
 ...
 <div className="mt-2 max-h-48 overflow-auto border rounded">{indicatorList.map(it=><div key={it.id} onClick={()=>setIndicatorId(it.id)} className={`px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 ${indicatorId===it.id?"bg-gray-100":""}`}><div className="flex justify-between"><div>{it.name}</div><span className={`text-[11px] px-2 rounded ${it.status==="proposed"?"bg-yellow-100 text-yellow-800":"bg-gray-100"}`}>{it.status}</span></div>{it.description&&<div className="text-xs text-gray-500">{it.description}</div>}</div>)}</div></div>
 ...
 <CreateIndicatorInlineModal open={createIndicatorOpen} onClose={()=>setCreateIndicatorOpen(false)} taxonomyDefault={taxonomyIds} onCreated={async id=>{await supabase.from("indicator_catalogue").update({status:"proposed"}).eq("id",
