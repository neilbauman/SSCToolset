...
const[title,setTitle]=useState(""),[desc,setDesc]=useState(""),[source,setSource]=useState(""),
[sourceUrl,setSourceUrl]=useState(""),[year,setYear]=useState<number|''>(""),
...
[rows,setRows]=useState<any[]>([]),[joinColumn,setJoinColumn]=useState(""),
[nameColumn,setNameColumn]=useState(""),[categoryCols,setCategoryCols]=useState<string[]>([]),
...
async function parseCSV(f:File){
 return new Promise<{headers:string[];rows:any[]}>((res,rej)=>Papa.parse(f,{
  header:true,dynamicTyping:true,skipEmptyLines:true,
  complete:r=>res({headers:r.meta.fields??Object.keys(r.data[0]??{}),rows:r.data.slice(0,300)}),
  error:rej}));
}
async function handleFile(e:any){
 const f=e.target.files?.[0];if(!f)return;
 const{headers,rows}=await parseCSV(f);setFile(f);setHeaders(headers);setRows(rows);
...
async function loadTaxonomy(){
 const {data}=await supabase.from("taxonomy_terms").select("id,name,category,sort_order").order("sort_order",{ascending:true});
 const cats=Array.from(new Set((data??[]).map(t=>t.category).filter(Boolean))).sort();
 setGroups(cats);setTerms(data??[]);
}
...
async function searchIndicators(){
 ...
 if(selectedTerm){
  const link=await supabase.from("indicator_taxonomy_links").select("indicator_id").eq("taxonomy_id",selectedTerm);
  const ids=(link.data??[]).map(i=>i.indicator_id);if(ids.length)query=query.in("id",ids);
 }
  ...
  <div className="flex flex-wrap gap-3">
   <div><label className={L}>Filter by Group</label>
    <select className={F} value={selectedGroup} onChange={e=>{setSelectedGroup(e.target.value);setSelectedTerm("");}}>
     <option value="">All</option>{groups.map(g=><option key={g}>{g}</option>)}</select></div>
   <div><label className={L}>Filter by Term</label>
    <select className={F} value={selectedTerm} onChange={e=>setSelectedTerm(e.target.value)}>
     <option value="">All</option>{terms.filter(t=>!selectedGroup||t.category===selectedGroup).map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
  </div>
   ...
   <div className="md:col-span-2"><label className={L}>Indicator</label>
    ...
    <div className="mt-2 max-h-48 overflow-auto border rounded">{indicatorList.map(it=>
     <div key={it.id} onClick={()=>setIndicatorId(it.id)} className={`px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 ${indicatorId===it.id?"bg-gray-100":""}`}>
      <div className="flex justify-between"><div>{it.name}</div>
       <span className={`text-[11px] px-2 rounded ${it.status==="proposed"?"bg-yellow-100 text-yellow-800":"bg-gray-100"}`}>{it.status}</span></div>
