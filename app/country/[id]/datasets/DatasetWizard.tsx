...
type DatasetWizardProps = { countryIso: string; onClose: () => void; onSaved: () => void };
...
export default function DatasetWizard({countryIso,onClose,onSaved}:DatasetWizardProps){
 ...
 [adminLevel,setAdminLevel]=useState("ADM0"),[nationalValue,setNationalValue]=useState("");
 const[file,setFile]=useState<File|null>(null),[headers,setHeaders]=useState<string[]>([]),[rows,setRows]=useState<any[]>([]),
 [joinColumn,setJoinColumn]=useState("pcode"),[valueColumn,setValueColumn]=useState("value"),[categoryMap,setCategoryMap]=useState<{code:string;label:string}[]>([]);
 ...
 async function parseCSV(f:File){return new Promise<{headers:string[];rows:any[]}>((res,rej)=>Papa.parse(f,{header:true,dynamicTyping:true,skipEmptyLines:true,
  complete:r=>res({headers:r.meta.fields??Object.keys(r.data[0]??{}),rows:r.data.slice(0,500)}),error:rej}));}
 async function handleFile(e:any){const f=e.target.files?.[0];if(!f)return;setFile(f);const{headers,rows}=await parseCSV(f);setHeaders(headers);setRows(rows);
  const vals=rows.map(r=>r[valueColumn]),uniq=new Set(vals.filter(Boolean)),numR=[...uniq].filter(v=>!isNaN(Number(v))).length/Math.max(uniq.size,1);
  if(numR<0.5&&uniq.size<=20){setDatasetType("categorical");setCategoryMap([...uniq].map(c=>({code:c,label:c})))}else setDatasetType("gradient");}
 ...
 async function saveAll(){setBusy(true);setError(null);
  try{const{data:meta,error:mErr}=await supabase.from("dataset_metadata").insert({
    title,description:desc,source,source_url:sourceUrl,year:year===""?null:Number(year),admin_level:adminLevel,data_type:datasetType,data_format:dataFormat,
    country_iso:countryIso,indicator_id:indicatorId??null}).select().single();if(mErr)throw mErr;const id=meta.id;
    ...
    if(datasetType==="gradient"){const d=rows.filter(r=>r[joinColumn]&&r[valueColumn]!=null).map(r=>({dataset_id:id,admin_pcode:String(r[joinColumn]).trim(),
      admin_level:adminLevel,value:dataFormat==="text"?null:Number(r[valueColumn]),text_value:dataFormat==="text"?String(r[valueColumn]):null}));
      if(d.length)await supabase.from("dataset_values").insert(d);}else{const maps=categoryMap.map(m=>({dataset_id:id,code:m.code,label:m.label,score:null}));
      if(maps.length)await supabase.from("dataset_category_maps").insert(maps);
      const cat=rows.map(r=>{const c=String(r[valueColumn]).trim(),m=categoryMap.find(x=>x.code===c);return{dataset_id:id,admin_pcode:String(r[joinColumn]).trim(),
      admin_level:adminLevel,category_code:c,category_label:m?.label??c}});if(cat.length)await supabase.from("dataset_values_cat").insert(cat);}
    setStep(5);onSaved();}
   ...
   <div className="p-5 space-y-4 overflow-y-auto">
    ...
    {step===1&&(<section className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <div><label className={L}>Type</label><select className={F} value={datasetType} onChange={e=>setDatasetType(e.target.value as any)}>
        ...
        <div><label className={L}>Admin Level</label><select className={F} value={adminLevel} onChange={e=>setAdminLevel(e.target.value)}>
          {["ADM0","ADM1","ADM2","ADM3","ADM4","ADM5"].map(a=><option key={a}>{a}</option>)}</select></div></div>
      {adminLevel==="ADM0"&&(<div><label className={L}>National Value ({dataFormat})</label>
        <input className={F} value={nationalValue} onChange={e=>setNationalValue(e.target.value)}/></div>)}
    ...
    {/* STEP 2 */}
    {step===2&&adminLevel!=="ADM0"&&(<section className="space-y-4">
    ...
    {/* STEP 3 */}
    {step===3&&datasetType==="categorical"&&(<section className="space-y-2">
      ...
      {!categoryMap.length&&<p className="text-xs text-gray-500">No categories detected.</p>}
    </section>)}
    ...
    {step===4&&(<section className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <div className="md:col-span-2"><label className={L}>Indicator</label>
          ...
          <div className="mt-2 max-h-48 overflow-auto border rounded">
            {indicatorList.map(it=><div key={it.id} onClick={()=>setIndicatorId(it.id)} className={`px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 ${indicatorId===it.id?"bg-gray-100":""}`}>
              ...
              {it.description&&<div className="text-xs text-gray-500">{it.description}</div>}</div>)}
          </div></div>
