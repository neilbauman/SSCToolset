...
const [datasets,setDatasets]=useState<any[]>([]),[preview,setPreview]=useState<any[]>([]),
[selectedId,setSelectedId]=useState<string|null>(null),[wizardOpen,setWizardOpen]=useState(false),
[loading,setLoading]=useState(false),[sortKey,setSortKey]=useState("title"),[sortAsc,setSortAsc]=useState(true);

async function loadDatasets(){
 const{data,error}=await supabase.from("view_country_datasets").select("*").eq("country_iso",params.id);
 if(!error&&data){
   const sorted=[...data].sort((a,b)=>String(a.title).localeCompare(String(b.title)));
   setDatasets(sorted);
 }
}
...
<tr>
{["title","year","admin_level","data_type","data_format","indicator_name","taxonomy_category","taxonomy_term"].map((key,i)=>
 <th key={i} onClick={()=>sortBy(key)} className="px-3 py-2 text-left cursor-pointer select-none">
  {["Title","Year","Admin","Type","Format","Indicator","Taxonomy Category","Taxonomy Term"][i]}
...
<tbody>
{datasets.map(d=>(
<tr key={d.id} onClick={()=>{setSelectedId(d.id);loadPreview(d.id);}}
 className={`cursor-pointer ${selectedId===d.id?"bg-[color:var(--gsc-beige)] font-semibold":""}`}>
 <td className="px-3 py-2">{d.title}</td><td>{d.year||"-"}</td><td>{d.admin_level}</td>
 <td>{d.data_type}</td><td>{d.data_format}</td><td>{d.indicator_name||"-"}</td>
 <td>{d.taxonomy_category||"-"}</td><td>{d.taxonomy_term||"-"}</td>
 <td className="flex gap-2 justify-center">
  <button className="p-1 border rounded hover:bg-gray-50"><Edit3 className="w-4 h-4 text-gray-600"/></button>
  <button className="p-1 border rounded hover:bg-gray-50"><Trash2 className="w-4 h-4 text-[color:var(--gsc-red)]"/></button>
 </td>
</tr>))}
</tbody></table></div>
...
Data Preview — {datasets.find(x=>x.id===selectedId)?.title}</div>
...
<tr><th className="text-left px-2 py-1 border-b">Admin PCode</th>
<th className="text-left px-2 py-1 border-b">Admin Level</th>
<th className="text-left px-2 py-1 border-b">Category</th>
<th className="text-left px-2 py-1 border-b">Value</th></tr></thead>
<tbody>{preview.map((r,i)=>(
<tr key={i}><td className="px-2 py-1 border-b">{r.admin_pcode}</td>
<td className="px-2 py-1 border-b">{r.admin_level||"-"}</td>
<td className="px-2 py-1 border-b">{r.category_label||"-"}</td>
<td className="px-2 py-1 border-b">{r.value??"-"}</td></tr>))}</tbody></table>)}
</div></div>)}

{wizardOpen&&<DatasetWizard countryIso={params.id}
