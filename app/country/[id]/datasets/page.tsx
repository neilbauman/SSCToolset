...
import { Loader2, PlusCircle, ArrowUpDown, Search, Edit3, Trash2 } from "lucide-react";
...
export default function CountryDatasetsPage() {
  ...
  // Load metadata + taxonomy (category/name from taxonomy_terms)
  useEffect(() => {
    if (!iso) return;
    (async () => {
      setLoading(true);
      ...
      if (indicatorIds.length) {
        ...
        if (termIds.length) {
          const { data: terms } = await supabase
            ...
            .in("id", termIds);
          (terms || []).forEach((t: any) => (termsById[t.id] = { category: t.category, name: t.name }));
        }
        const map: Record<string, TaxInfo> = {};
        (links || []).forEach((l: any) => {
          const t = termsById[l.taxonomy_id];
          if (!t) return;
          // if multiple terms, prefer the first one encountered; you can also concatenate if needed
          if (!map[l.indicator_id]) map[l.indicator_id] = { category: t.category, term: t.name };
        });
        setTax(map);
      } else {
        setTax({});
  ...
  async function cascadeDelete(d: DatasetMeta) {
    // delete child rows then metadata
  ...
  return (
    <SidebarLayout
      headerProps={{
        title: `${iso} – Datasets`,
        group: "country-config",
        description: "Catalogue of raw datasets. Select a row to preview the raw uploaded data below.",
        tool: "Dataset Manager",
        breadcrumbs: (
          <Breadcrumbs items={[{ label: "Countries", href: "/country" }, { label: iso, href: `/country/${iso}` }, { label: "Datasets" }]} />
        ),
      }}
    >
      {/* Toolbar */}
              ...
              {[
                ...
                <th key={key} className="px-2 py-2 whitespace-nowrap">
                  {key === "actions" ? (
                    ...
                    <button className="inline-flex items-center gap-1" onClick={() => toggleSort(key as SortKey)} title={`Sort by ${label}`}>
                      {label}
            ...
            {loading ? (
              ...
              filtered.map((d) => {
                ...
                return (
                  ...
                  >
                    <td className="px-2 py-2 font-medium cursor-pointer" onClick={() => setSelected(isSel ? null : d)}>
                      {d.title}
                    </td>
      ...
      {/* Inline preview (raw uploaded data) */}
      {selected && (
      ...
      {editing && (
        <EditDatasetModal
          ...
          onSaved={(updated) => {
            setRows((prev) => prev.map((r) => (r.id === updated.id ? (updated as DatasetMeta) : r)));
            setEditing(null);
          }}
        />
      )}
