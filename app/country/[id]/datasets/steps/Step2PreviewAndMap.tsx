...
export default function Step2PreviewAndMap({
  ...
  // Load a few admin pcodes for guidance
  useEffect(() => {
    if (isAdm0) return;
    (async () => {
      setLoading(true);
  ...
  const canContinue = isAdm0 || (hasHeaders && !!joinField);
  ...
  return (
    <div className="space-y-4 text-sm text-[var(--gsc-gray)]">
      ...
      <p>
        Review your dataset below and identify which column contains the{" "}
        <strong>join field</strong> (typically the administrative Pcode).
      </p>

      {/* ADM0 path */}
      {isAdm0 && (
        <div className="rounded-xl border bg-white p-4 space-y-2">
          <p>This dataset represents a national ADM0 metric — no join field needed.</p>
          <div className="flex items-center gap-2 text-[var(--gsc-green)]">
            <Check className="h-4 w-4" /> ADM0 confirmed
          </div>
        </div>
      )}
